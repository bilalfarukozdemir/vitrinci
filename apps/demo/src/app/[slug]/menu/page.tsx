...
export function generateStaticParams() {
  return SLUGLAR.filter((slug) => DEMOLAR[slug]?.menu).map((slug) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  if (!demo) return {};
  return { title: `Menü — ${demo.isletme.ad}` };
}

export default async function MenuSayfasi({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const demo = DEMOLAR[slug];
  ...
  return (
    <main className="menu-sayfa">
      <header className="menu-tepe">
        <div className="sar">
          <a className="menu-geri" href={`/${slug}`}>
            ← {isletme.ad}
          </a>
